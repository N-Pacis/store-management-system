    ...
    <?php
           ...
           if(!$con){
               echo "Failed to connect due to.".mysqli_connect_error();
                ...
                $searchTable = mysqli_query($con,"SELECT * from stk_outgoing");
                $searchQuery = "SELECT sum(quantity) from stk_outgoing";
                ...
                if(!$result){
                    ...
                    if(mysqli_num_rows($searchTable)>0){
                        ...
                        echo "<div class='success-div'>".$row['sum(quantity)']."&nbspOutgoing products.</div>";
                        echo "<br><a href='reports.php' class='register-link'>Back to Reports?</a>";
                    }
                    else{
                        echo "<div class='error-div'><i class='far fa-sad-tear'></i>No Outgoing product found in the database.<a href='reports.php'>Back to Reports?</a></div>";
                        echo "<br><a href='reports.php' class='register-link'>Back to Reports?</a>";
                    }
                }
           }
