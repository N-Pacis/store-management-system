    ...
    <?php
           ...
           if(!$con){
               echo "Failed to connect due to.".mysqli_connect_error();
                ...
                $searchQuery = "SELECT usr.firstName,usr.lastName,usr.email,usr.username,prod.product_Name,prod.brand,prod.supplier,prod.supplier_phone,outgo.quantity from stk_outgoing outgo INNER JOIN stk_users usr on outgo.userId=usr.userId INNER JOIN stk_products prod on outgo.productId=prod.productId";
