    ...
    <?php
          ...
          if(!$con){
             echo "<div class='error-div'><i class='far fa-times-circle'></i>Failed to connect due to.".mysqli_connect_error()."</div>";
              ...
              $inventory = $_GET['record'];
      
              $findQuery = "SELECT * FROM stk_inventory where inventory_id=$inventory";
              $result = mysqli_query($con,$findQuery);
              if(!$result){
                echo "<div class='error-div'><i class='far fa-times-circle'></i>Unable to find the specified incoming product.</div>";
              }
      
              else{
                    $rowInventory = mysqli_fetch_assoc($result);
                    if($rowInventory != $row["userId"]){
                        header("location:view_incoming_products.php");
                    }
                    else{
                       $updateQuery = "DELETE FROM stk_inventory where inventory_id=$inventory";
                        $result = mysqli_query($con,$updateQuery);

                        if(!$result){
                            echo "<div class='error-div'><i class='far fa-times-circle'></i>Unable to delete incoming product due to.".mysqli_error($con)."</div>";
                        }
